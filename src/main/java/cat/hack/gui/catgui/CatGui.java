...
import cat.hack.core.manager.client.ConfigManager;
import cat.hack.features.cmd.Command;
import cat.hack.features.modules.Module;
import cat.hack.features.modules.client.CatHackGui;
import cat.hack.gui.font.FontRenderers;
import cat.hack.gui.catgui.components.*;
...
import cat.hack.utility.render.animation.EaseOutBack;
import cat.hack.gui.catgui.components.BooleanComponent;
 ...
 *
 * @Copyright by thekoteczek
 **/
public class CatGui extends Screen {
    ...
    private static CatGui INSTANCE;
    ...
    public static CatGui getInstance() {
        if (INSTANCE == null) {
            INSTANCE = new CatGui();
        }
        return INSTANCE;
    ...
    public static CatGui getCatGui() {
    ...
    public void renderGui(DrawContext context, int mouseX, int mouseY, float partialTicks) {
        ...
        Render2DEngine.drawRound(context.getMatrices(), main_posX, main_posY, main_width, main_height, 9f, CatHackGui.getColorByTheme(0));

        // Плита с лого / Main GUI logo
        Render2DEngine.drawRound(context.getMatrices(), main_posX + 5, main_posY + 5, 90, 30, 7f, CatHackGui.getColorByTheme(1));

        context.getMatrices().push();
        context.getMatrices().scale(0.85f, 0.85f, 1);
        context.getMatrices().translate((main_posX + 10) / 0.85, (main_posY + 15) / 0.85, 0);
        FontRenderers.thglitch.drawString(context.getMatrices(), "CATHACK", 0, 0, CatHackGui.getColorByTheme(2).getRGB());
        context.getMatrices().translate(-(main_posX + 10) / 0.85, -(main_posY + 15) / 0.85, 0);
        context.getMatrices().scale(1, 1, 1);
        context.getMatrices().pop();

        FontRenderers.settings.drawString(context.getMatrices(), "recode v" + CatHack.VERSION, main_posX + 91 - (FontRenderers.settings.getStringWidth("recode v" + CatHack.VERSION)), main_posY + 30, CatHackGui.getColorByTheme(3).getRGB());

        // Левая плита под категриями
        Render2DEngine.drawRound(context.getMatrices(), main_posX + 5, main_posY + 40, 90, 120, 7f, CatHackGui.getColorByTheme(4));

        // Выбор между CfgManager и FriendManager
        if (currentMode == CurrentMode.Modules) {
            Render2DEngine.drawRound(context.getMatrices(), main_posX + 20, main_posY + 195, 60, 20, 4f, CatHackGui.getColorByTheme(4));
        } else if (currentMode == CurrentMode.CfgManager) {
            Render2DEngine.drawGradientRound(context.getMatrices(), main_posX + 20, main_posY + 195, 60, 20, 4f, CatHackGui.getColorByTheme(4), CatHackGui.getColorByTheme(4), CatHackGui.getColorByTheme(5), CatHackGui.getColorByTheme(5));
        } else {
            Render2DEngine.drawGradientRound(context.getMatrices(), main_posX + 20, main_posY + 195, 60, 20, 4f, CatHackGui.getColorByTheme(5), CatHackGui.getColorByTheme(5), CatHackGui.getColorByTheme(4), CatHackGui.getColorByTheme(4));
        }

        Render2DEngine.drawRound(context.getMatrices(), main_posX + 49.5f, main_posY + 197, 1, 16, 0.5f, CatHackGui.getColorByTheme(6));

        FontRenderers.mid_icons.drawString(context.getMatrices(), "u", main_posX + 20, main_posY + 196, currentMode == CurrentMode.CfgManager ? CatHackGui.getColorByTheme(2).getRGB() : new Color(0x8D8D8D).getRGB());
        FontRenderers.mid_icons.drawString(context.getMatrices(), "v", main_posX + 54, main_posY + 197, currentMode == CurrentMode.FriendManager ? CatHackGui.getColorByTheme(2).getRGB() : new Color(0x8D8D8D).getRGB());
        ...
        if (first_open) {
            category_animation = 1;
            Render2DEngine.drawRound(context.getMatrices(), (float) (main_posX + 8), (float) CategoryY + slider_y, 84, 15, 2f, CatHackGui.getColorByTheme(7));
            first_open = false;
        } else {
            if (currentMode == CurrentMode.Modules)
                Render2DEngine.drawRound(context.getMatrices(), (float) (main_posX + 8), (float) (Render2DEngine.interpolate(CategoryY, prevCategoryY, category_animation)) + slider_y, 84, 15, 2f, CatHackGui.getColorByTheme(7));
        }
        ...
        if (currentMode != CurrentMode.Modules) {
            ...
            Render2DEngine.drawRound(context.getMatrices(), main_posX + 100, (float) main_posY + 40, (float) 295, (float) main_height - 44, 4f, CatHackGui.getColorByTheme(7));
            ...
            Render2DEngine.draw2DGradientRect(context.getMatrices(), main_posX + 102, main_posY + 34, main_posX + 393, main_posY + 60, new Color(25, 20, 30, 0), CatHackGui.getColorByTheme(7), new Color(25, 20, 30, 0), new Color(37, 27, 41, 245));
            Render2DEngine.draw2DGradientRect(context.getMatrices(), main_posX + 102, main_posY + main_height - 35, main_posX + 393, main_posY + main_height, CatHackGui.getColorByTheme(7), new Color(25, 20, 30, 0), CatHackGui.getColorByTheme(7), new Color(37, 27, 41, 0));
            Render2DEngine.popWindow();
        }
        ...
        if (currentMode == CurrentMode.CfgManager) {
            ...
            FontRenderers.modules.drawCenteredString(context.getMatrices(), "+", main_posX + 378, main_posY + 16, CatHackGui.getColorByTheme(2).getRGB());
        }
        if (currentMode == CurrentMode.FriendManager) {
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int clickedButton) {
        ...
        if (isHoveringItem(main_posX + 105, main_posY + 14, 11, 11, (float) mouseX, (float) mouseY)) {
            try {
                net.minecraft.util.Util.getOperatingSystem().open(new File("CatHackPL/configs/").toURI());
            } catch (Exception e) {
        ...
        if (isHoveringItem(main_posX + 250, main_posY + 15, 140, 10, (float) mouseX, (float) mouseY) && currentMode == CurrentMode.Modules) {
            searching = true;
            CatHack.currentKeyListener = CatHack.KeyListening.CatGui;
        }
    ...
    public void keyTyped(String typedChar, int keyCode) throws IOException {
        if (CatHack.currentKeyListener != CatHack.KeyListening.Sliders && CatHack.currentKeyListener != CatHack.KeyListening.CatGui)
            return;
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double horizontalAmount, double verticalAmount) {
        ...
        if (scroll_lock) {
            ...
            if (isHoveringItem(main_posX + 200, main_posY + 40, main_posX + 395, main_posY - 5 + main_height, (float) mouseX, (float) mouseY))
                scroll += dWheel * CatHackGui.scrollSpeed.getValue();
            else {
                components.forEach(component -> component.scrollElement(dWheel * CatHackGui.scrollSpeed.getValue()));
            }
            configs.forEach(component -> component.scrollElement(dWheel * CatHackGui.scrollSpeed.getValue()));
            friends.forEach(component -> component.scrollElement(dWheel * CatHackGui.scrollSpeed.getValue()));
        }
