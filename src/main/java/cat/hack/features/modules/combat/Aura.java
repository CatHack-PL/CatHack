...
import cat.hack.utility.interfaces.IOtherClientPlayerEntity;
import cat.hack.utility.math.MathUtility;
import cat.hack.utility.player.InteractionUtility;
import cat.hack.utility.player.InventoryUtility;
import cat.hack.utility.player.PlayerUtility;
import cat.hack.utility.player.SearchInvResult;
import cat.hack.utility.render.Render2DEngine;
import cat.hack.utility.render.Render3DEngine;
import cat.hack.utility.render.animation.CaptureMark;
...
import static cat.hack.utility.math.MathUtility.random;
...
public class Aura extends Module {
    ...
    public final Setting<ESP> esp = new Setting<>("ESP", ESP.CatHack);
    public final Setting<SettingGroup> espGroup = new Setting<>("ESPSettings", new SettingGroup(false, 0), v -> esp.is(ESP.CatHackV2));
    public final Setting<Integer> espLength = new Setting<>("ESPLength", 14, 1, 40, v -> esp.is(ESP.CatHackV2)).addToGroup(espGroup);
    public final Setting<Integer> espFactor = new Setting<>("ESPFactor", 8, 1, 20, v -> esp.is(ESP.CatHackV2)).addToGroup(espGroup);
    public final Setting<Float> espShaking = new Setting<>("ESPShaking", 1.8f, 1.5f, 10f, v -> esp.is(ESP.CatHackV2)).addToGroup(espGroup);
    public final Setting<Float> espAmplitude = new Setting<>("ESPAmplitude", 3f, 0.1f, 8f, v -> esp.is(ESP.CatHackV2)).addToGroup(espGroup);
    ...
    public void onUpdate(PlayerUpdateEvent e) {
        ...
        if (mc.player.isUsingItem() && pauseWhileEating.getValue())
            return;
        if(pauseBaritone.getValue() && CatHack.baritone){
    ...
    public float getAttackCooldownProgressPerTick() {
        return (float) (1.0 / mc.player.getAttributeValue(EntityAttributes.GENERIC_ATTACK_SPEED) * (20.0 * CatHack.TICK_TIMER * (tpsSync.getValue() ? Managers.SERVER.getTPSFactor() : 1f)));
    }
    ...
    public void onRender3D(MatrixStack stack) {
        ...
        switch (esp.getValue()) {
            ...
            case CatHackV2 -> Render3DEngine.renderGhosts(espLength.getValue(), espFactor.getValue(), espShaking.getValue(), espAmplitude.getValue(), target);
            case CatHack -> Render3DEngine.drawTargetEsp(stack, target);
        }
    ...
    public enum ESP {
        Off, CatHack, NurikZapen, CelkaPasta, CatHackV2
    }
