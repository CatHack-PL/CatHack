...
import cat.hack.injection.accesors.IInteractionManager;
import cat.hack.features.modules.Module;
import cat.hack.setting.Setting;
import cat.hack.utility.interfaces.IEntity;
...
import static cat.hack.utility.player.MovementUtility.isMoving;
...
public class Speed extends Module {
    ...
    private cat.hack.utility.Timer elytraDelay = new cat.hack.utility.Timer();
    private cat.hack.utility.Timer startDelay = new cat.hack.utility.Timer();
    ...
    public void onSync(EventSync e) {
        ...
        if (mode.getValue() == Mode.MatrixJB) {
            ...
            if (MovementUtility.isMoving() && closeToGround && mc.player.fallDistance <= 0) {
                ...
                CatHack.TICK_TIMER = matrixJBSpeed.getValue();
    ...
    public void modifyVelocity(EventPlayerTravel e) {
        if (mode.getValue() == Mode.GrimEntity && !e.isPre() && CatHack.core.getSetBackTime() > 1000) {
            for (PlayerEntity ent : Managers.ASYNC.getAsyncPlayers()) {
                if (ent != mc.player && mc.player.squaredDistanceTo(ent) <= 2.25) {
                    float p = mc.world.getBlockState(((IEntity) mc.player).catHack_PL$getVelocityBP()).getBlock().getSlipperiness();
        ...
        if ((mode.is(Mode.GrimEntity2) || mode.is(Mode.GrimCombo)) && !e.isPre() && CatHack.core.getSetBackTime() > 1000 && MovementUtility.isMoving()) {
            int collisions = 0;
    ...
    public void onTick(EventTick e) {
        ...
        if ((mode.is(Mode.GrimIce) || mode.is(Mode.GrimCombo)) && mc.player.isOnGround()) {
            BlockPos pos = ((IEntity) mc.player).catHack_PL$getVelocityBP();
    ...
    public void onMove(EventMove event) {
        ...
        if (MovementUtility.isMoving()) {
            CatHack.TICK_TIMER = useTimer.getValue() ? 1.088f : 1f;
