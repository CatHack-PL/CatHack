...
import cat.hack.events.impl.PostPlayerUpdateEvent;
...
public class Timer extends Module {
    ...
    public void onUpdate() {
        switch (mode.getValue()) {
            case Normal -> CatHack.TICK_TIMER = speed.getValue();
            case Matrix -> {
                ...
                CatHack.TICK_TIMER = Math.max(speed.getValue(), 1f);
            ...
            case Grim -> {
                if (energy <= 0 || !isKeyPressed(boostKey) || CatHack.core.getSetBackTime() < 2000) {
                    CatHack.TICK_TIMER = 1f;
                    return;
                }
    ...
    public void onPacketReceive(PacketEvent.Receive e) {
        if (mode.is(Mode.Grim)) {
            if (e.getPacket() instanceof CommonPingS2CPacket
                    && CatHack.core.getSetBackTime() > 2000) {
