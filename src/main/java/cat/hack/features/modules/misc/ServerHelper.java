...
public class ServerHelper extends Module {
        ...
        if (event.getPacket() instanceof GameMessageS2CPacket pac) {
            if (spek.getValue()) {
                ...
                if (content.contains("спек") || content.contains("ызус") || content.contains("spec") || content.contains("spek") || content.contains("ызул")) {
                    String name = CatUtility.solveName(pac.content().getString());
            ...
            if (photomath.getValue())
                if (pac.content().getString().contains("Решите: ") && Objects.equals(CatUtility.solveName(pac.content().getString()), "FATAL ERROR"))
                    try {
            ...
            if (airDropWay.getValue() && pac.content().getString().contains("Аирдроп")) {
                try {
                    ...
                    CatHack.gps_position = new BlockPos(xCord, yCord, zCord);
    ...
    public boolean check(String checkstring) {
        return checktimer.passedMs(3000) && (Objects.equals(CatUtility.solveName(checkstring), "FATAL ERROR"));
    }
