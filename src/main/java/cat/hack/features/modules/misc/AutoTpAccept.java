...
public class AutoTpAccept extends Module {
        ...
        if (event.getPacket() instanceof GameMessageS2CPacket) {
            ...
            if (packet.content().getString().contains("телепортироваться") || packet.content().getString().contains("tpaccept")) {
                if (onlyFriends.getValue()) {
                    if (Managers.FRIEND.isFriend(CatUtility.solveName(packet.content().getString()))) {
    ...
    public void acceptRequest(String name) {
        if (grief.getValue()) mc.getNetworkHandler().sendChatCommand("tpaccept " + CatUtility.solveName(name));
