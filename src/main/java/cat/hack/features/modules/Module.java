package cat.hack.features.modules;
...
import cat.hack.core.manager.client.CommandManager;
...
import cat.hack.features.modules.client.Windows;
import cat.hack.features.modules.client.UnHook;
...
import static cat.hack.CatHack.LOGGER;
import static cat.hack.core.Managers.NOTIFICATION;
...
public abstract class Module {
    ...
    protected void sendPacketSilent(Packet<?> packet) {
        if (mc.getNetworkHandler() == null) return;
        CatHack.core.silentPackets.add(packet);
        mc.getNetworkHandler().sendPacket(packet);
    }
    ...
    public void enable() {
        ...
        if (isOn()) CatHack.EVENT_BUS.subscribe(this);
        if (fullNullCheck()) return;

        LogUtils.getLogger().info("[CatHack] enabled " + this.getName());
    ...
    public void disable() {
        try {
            CatHack.EVENT_BUS.unsubscribe(this);
        ...
        LOGGER.info("[CatHack] disabled {}", getName());
