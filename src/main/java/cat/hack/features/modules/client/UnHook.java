...
public class UnHook extends Module { // Йо фабос, засунь в о4ко себе фалос
            ...
            mc.executeSync(() -> {
                ...
                try {
                    ...
                    while ((line = reader.readLine()) != null) {
                        if(line.contains("cathack") || line.contains("CatHack") || line.contains("$$") || line.contains("\\______/")
                                || line.contains("By thekoteczek, 06ED") || line.contains("\u26A1") || line.contains("cat.hack"))
                            continue;
                        lines.add(line);
                    }
                    fis.close();
    ...
    public void onDisable() {
        ...
        // Rename cfg dir back
        try {
            new File("XaeroWaypoints_BACKUP092738").renameTo(new File("CatHackPL"));
        } catch (Exception e) {
        }
    }
}
