...
import cat.hack.features.modules.combat.AutoAnchor;
import cat.hack.features.modules.combat.AutoCrystal;
...
import cat.hack.utility.render.animation.EaseOutCirc;
...
public class TargetHud extends HudElement {
    ...
    private final Setting<ModeEn> Mode = new Setting<>("Mode", ModeEn.CatHack);
    ...
    public void onEnable() {
        try {
            custom = CatUtility.getCustomImg("thud");
        } catch (Exception e) {
            sendMessage(".minecraft -> CatHackPL -> misc -> images -> thud.png");
        }
    }
    ...
    public void onRender2D(DrawContext context) {
        ...
        if (animation.getAnimationd() > 0) {
            ...
            switch (Mode.getValue()) {
                case CatHack -> renderCatHack(context, health, animationFactor);
    ...
    private void renderCatHack(DrawContext context, float health, float animationFactor) {
        float hurtPercent = target.hurtTime / 6f;
