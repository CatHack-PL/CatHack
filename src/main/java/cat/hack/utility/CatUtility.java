...
import static cat.hack.core.manager.client.ConfigManager.IMAGES_FOLDER;
import static cat.hack.features.modules.Module.mc;

public final class CatUtility {
    ...
    public static @NotNull String getAuthors() {
        List<String> names = CatHack.MOD_META.getAuthors()
                .stream()
                .map(Person::getName)
                .toList();
    ...
    public static void syncVersion() {
        try {
            if (!new BufferedReader(new InputStreamReader(new URL("https://raw.githubusercontent.com/CatHackPL/chUtil/main/syncVersion121.txt").openStream())).readLine().equals(CatHack.VERSION))
                CatHack.isOutdated = true;
    ...
    public static void parseStarGazer() {
        ...
        try {
            for (int page = 1; page <= 3; page++) {
                URL url = new URL("https://api.github.com/repos/CatHackPL/CatHack/stargazers?per_page=100&page=" + page);
    ...
    public static void syncContributors() {
        try {
            URL list = new URL("https://raw.githubusercontent.com/CatHack/chUtil/main/chTeam.txt");
            BufferedReader in = new BufferedReader(new InputStreamReader(list.openStream(), StandardCharsets.UTF_8));
            String inputLine;
            int i = 0;
            while ((inputLine = in.readLine()) != null) {
                CatHack.contributors[i] = inputLine.trim();
                i++;
            }
            in.close();
        } catch (Exception e) {
    ...
    public static void parseCommits() {
        try {
            URL url = new URL("https://api.github.com/repos/CatHackPL/CatHack/commits?per_page=50");
            BufferedReader in = new BufferedReader(new InputStreamReader(url.openStream(), StandardCharsets.UTF_8));

            changeLog.add("Changelog [Recode; Date: " + CatHack.BUILD_DATE + "; GitHash:" + CatHack.GITHUB_HASH + "]");
            changeLog.add("\n");
