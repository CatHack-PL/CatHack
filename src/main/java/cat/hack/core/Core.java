...
import cat.hack.core.manager.client.MacroManager;
...
import static cat.hack.features.modules.Module.fullNullCheck;
...
public final class Core {
    ...
    public void onTick(PlayerUpdateEvent event) {
        ...
        for (PlayerEntity p : mc.world.getPlayers()) {
            if (p.isDead() || p.getHealth() == 0)
                CatHack.EVENT_BUS.post(new EventDeath(p));
        }
    ...
    public void drawGps(DrawContext e) {
        if (CatHack.gps_position != null) {
            float dst = getDistance(CatHack.gps_position);
            float xOffset = mc.getWindow().getScaledWidth() / 2f;
            float yOffset = mc.getWindow().getScaledHeight() / 2f;
            float yaw = getRotations(new Vec2f(CatHack.gps_position.getX(), CatHack.gps_position.getZ())) - mc.player.getYaw();
