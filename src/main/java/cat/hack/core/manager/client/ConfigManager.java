...
public class ConfigManager implements IManager {
    public static final String CONFIG_FOLDER_NAME = "CatHackPL";
    ...
    public static @NotNull String getConfigDate(String name) {
        File file = new File(CONFIGS_FOLDER, name + ".ch");
        if (!file.exists())
            return "none";
    ...
    public void loadCloud(String name) {
        ...
        try (BufferedInputStream in = new BufferedInputStream(new URL("https://raw.githubusercontent.com/CatHackPL/chUtil/main/configs/" + name + ".ch").openStream());
             FileOutputStream fileOutputStream = new FileOutputStream(new File(CONFIGS_FOLDER, name + ".ch"))) {
    ...
    public void loadModuleOnly(String name, Module module) {
        ...
        if (module.isEnabled()) {
            CatHack.EVENT_BUS.unsubscribe(module);
        ...
        if (module.isEnabled())
            CatHack.EVENT_BUS.subscribe(module);
    }
    ...
    private void parseModule(JsonObject object, String category) throws NullPointerException {
        ...
        for (Setting setting : module.getSettings()) {
            try {
                ...
                LogUtils.getLogger().warn("[CatHack] Module: " + module.getName() + " Setting: " + setting.getName() + " Error: ");
                e.printStackTrace();
            }
        }
    }
    ...
    public List<String> getConfigList() {
        ...
        if (CONFIGS_FOLDER.listFiles() != null) {
            for (File file : Arrays.stream(Objects.requireNonNull(CONFIGS_FOLDER.listFiles())).filter(f -> f.getName().endsWith(".ch")).toList()) {
                list.add(file.getName().replace(".ch", ""));
            }
        }
        return list;
    }
    ...
    public List<String> getCloudConfigs() {
        List<String> list = new ArrayList<>();
        try {
            URL url = new URL("https://raw.githubusercontent.com/CatHackPL/chUtil/main/cloudConfigs.txt");
    ...
    public void saveCurrentConfig() {
        ...
        try {
            if (file.exists()) {
                ...
                writer.write(currentConfig.getName().replace(".ch", ""));
                writer.close();
            } else {
                file.createNewFile();
                FileWriter writer = new FileWriter(file);
    ...
    public File getCurrentConfig() {
        ...
        currentConfig = new File(CONFIGS_FOLDER, name + ".ch");
