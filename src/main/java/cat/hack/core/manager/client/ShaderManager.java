package cat.hack.core.manager.client;

import com.mojang.blaze3d.platform.GlStateManager;
import com.mojang.blaze3d.systems.RenderSystem;
import cat.hack.utility.render.Render3DEngine;
import cat.hack.utility.render.shaders.satin.api.managed.ManagedShaderEffect;
import cat.hack.utility.render.shaders.satin.api.managed.ShaderEffectManager;
...
import cat.hack.features.modules.render.Shaders;
import cat.hack.utility.interfaces.IShaderEffect;
...
public class ShaderManager implements IManager {
    ...
    private CatHackFramebuffer shaderBuffer;
    ...
    public void renderShaders() {
        if (DEFAULT == null) {
            shaderBuffer = new CatHackFramebuffer(mc.getFramebuffer().textureWidth, mc.getFramebuffer().textureHeight);
            reloadShaders();
        }
    ...
    public void reloadShaders() {
        DEFAULT = ShaderEffectManager.getInstance().manage(Identifier.of("cathack", "shaders/post/outline.json"));
        SMOKE = ShaderEffectManager.getInstance().manage(Identifier.of("cathack", "shaders/post/smoke.json"));
        GRADIENT = ShaderEffectManager.getInstance().manage(Identifier.of("cathack", "shaders/post/gradient.json"));
        SNOW = ShaderEffectManager.getInstance().manage(Identifier.of("cathack", "shaders/post/snow.json"));
        FADE = ShaderEffectManager.getInstance().manage(Identifier.of("cathack", "shaders/post/fade.json"));

        FADE_OUTLINE = ShaderEffectManager.getInstance().manage(Identifier.of("cathack", "shaders/post/fade.json"), managedShaderEffect -> {
        ...
        DEFAULT_OUTLINE = ShaderEffectManager.getInstance().manage(Identifier.of("cathack", "shaders/post/outline.json"), managedShaderEffect -> {
            PostEffectProcessor effect = managedShaderEffect.getShaderEffect();
            if (effect == null) return;
        ...
        SMOKE_OUTLINE = ShaderEffectManager.getInstance().manage(Identifier.of("cathack", "shaders/post/smoke.json"), managedShaderEffect -> {
            PostEffectProcessor effect = managedShaderEffect.getShaderEffect();
            if (effect == null) return;
        ...
        GRADIENT_OUTLINE = ShaderEffectManager.getInstance().manage(Identifier.of("cathack", "shaders/post/gradient.json"), managedShaderEffect -> {
            PostEffectProcessor effect = managedShaderEffect.getShaderEffect();
            if (effect == null) return;
        ...
        SNOW_OUTLINE = ShaderEffectManager.getInstance().manage(Identifier.of("cathack", "shaders/post/snow.json"), managedShaderEffect -> {
            PostEffectProcessor effect = managedShaderEffect.getShaderEffect();
            if (effect == null) return;
    ...
    public static class CatHackFramebuffer extends Framebuffer {
        public CatHackFramebuffer(int width, int height) {
            super(false);
            RenderSystem.assertOnRenderThreadOrInit();
