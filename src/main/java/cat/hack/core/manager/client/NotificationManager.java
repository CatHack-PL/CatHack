...
import cat.hack.features.modules.client.Notifications;
...
public class NotificationManager implements IManager {
        ...
        if (SystemTray.isSupported()) {
            try {
                if (trayIcon == null) {
                    ...
                    trayIcon = new TrayIcon(image, "CatHack");
                    trayIcon.setImageAutoSize(true);
                    trayIcon.setToolTip("CatHack");
                    tray.add(trayIcon);
                }
    ...
    private void mac(final String message) {
        ...
        processBuilder.command("osascript", "-e", "display notification \"" + message + "\" with title \"CatHack\"");
        try {
    ...
    private void linux(final String message) {
        final ProcessBuilder processBuilder = new ProcessBuilder();
        processBuilder.command("notify-send", "-a", "CatHack", message);
