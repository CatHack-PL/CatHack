package cat.hack.core.manager.player;
...
import cat.hack.events.impl.EventPostTick;
import cat.hack.events.impl.PacketEvent;
import cat.hack.events.impl.TotemPopEvent;
import cat.hack.features.modules.Module;
import cat.hack.features.modules.combat.AntiBot;
    ...
    public void onPacketReceive(PacketEvent.Receive event) {
        ...
        if (event.getPacket() instanceof EntityStatusS2CPacket pac) {
            if (pac.getStatus() == EntityStatuses.USE_TOTEM_OF_UNDYING) {
                ...
                CatHack.EVENT_BUS.post(new TotemPopEvent((PlayerEntity) ent, popList.get(ent.getName().getString())));
            }
        }
    }
